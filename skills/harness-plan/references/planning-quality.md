...
8. non-trivial planning では TeamAgent / サブエージェント検証結果、または `サブエージェント未使用` fallback と 5 gate の結果を task context に残す
9. `team_validation_mode: unavailable` の plan は Required にしない。軽量 task だけ `not_required_lightweight` を許可する
