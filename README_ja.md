...
新規ユーザーは、今使っている tool を選び、証明済み tier の導線だけで導入します。
既存ユーザーは、cleanup の前に migration report を実行します。
...
Harness は agent 作業を、再実行できる loop にします。

1. `spec.md` と `Plans.md` が正本になる。
2. `/harness-plan` が要望を受入条件へ変換する。
3. `/harness-work` が TDD と検証つきで実装する。
4. `/harness-review` が実装とレビューを分離する。
5. `/harness-release` が検証済み evidence だけを出荷用にまとめる。
...
3. 小さな plan を作り、`Plans.md` を確認する。
4. 最小の `harness-work` slice を実行する。
5. review を走らせ、検証出力を残す。
...
Breezing、Codex companion review、OpenCode bootstrap、harness-mem は、基本の
trigger path が見えてから使います。candidate host は、それぞれの smoke gate
が通るまで research docs に留めます。
...
新規ユーザーは [tool-first onboarding](docs/onboarding/index.md) から始めてください。
既存ユーザーは report-first の [migration check](docs/onboarding/migration.md) を先に実行してください。
Claude-first の plugin install と Codex CLI の setup 導線を、候補 host の調査枠と分けて案内します。
...
| R03 | 保護パスへのシェル書き込み | 既定は**拒否**、TOML break-glass の `.env` / `.env.*` だけ**確認**可 |
...
これは handoff integration であり、Cursor adapter サポートではありません。
Cursor 固有の bootstrap / workflow smoke が通るまでは candidate のままです。
